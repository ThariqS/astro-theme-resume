...
One of the biggest advancements in LLMs this year has been in the field of interpretability- the ability to understand what a LLM is “thinking” and to steer its generation process.

The most famous example of this is probably Anthropic’s [Golden Gate Claude](https://www.anthropic.com/news/golden-gate-claude), which is a model that can be steered to generate text in a particular style. This work isn’t limited to text, it's also happening with [images](https://featurelab.xyz/), [voice](https://x.com/GopalKRaman/status/1832200457621557594/video/1) and even [protein models](https://www.nature.com/articles/s41587-024-0206-x).

But in contrast to the excitement amongst researchers, AI developers seem less enthused or even confused. 

*“Why can’t we just prompt that?”* is a question I've heard often. 

In this post I dive into how we might use interpretability & steering features in our applications.
...
The problem is that RLHF famously has side effects. It can lead to “false refusals” where the AI refuses to do something that ican do (the llama paper cites 1-5% of new false refusals), and sometimes degrades quality, e.g. RLHF to be more concise, can also make a model respond with code comments that just say to fill in the code.
...
Even better, steering happens at inference time unlike RLHF, which means that API devs should be able to choose how the model is steered more specifically, vs having to rely on the model providers making large blanket choices that affect everyone via RLHF in post-training.
...
Given that there might tens or hundreds of thousands of features, it seems likely that some will be mislabeled or misunderstood (broadly the research field addressing this is called autointerp).

### 3. Activating other features & circuits

Some features activate other features in ways that are complex to understand. We may be introducing other side effects while activating a feature, in the same way that RLHF introduces side effects. There has been no wide scale use of feature steering (unless Anthropic is doing it under the hood), and so it’s hard to tell what side effects they might produce.
